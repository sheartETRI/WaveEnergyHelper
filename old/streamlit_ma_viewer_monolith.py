...
RSI_OVERSOLD = 30
RSI_MIDLINE = 50
RSI_SMOOTHING_LENGTH = 2
RSI_SMOOTHING_TYPE = "SMA"
...
def add_macd(df, fast=MACD_FAST, slow=MACD_SLOW, signal=MACD_SIGNAL):
    """Adds traditional MACD(12,26,9) columns."""
    if df is None or df.empty:
        return df
...
def add_rsi(df, length=RSI_LENGTH, overbought=RSI_OVERBOUGHT, oversold=RSI_OVERSOLD,
            smoothing_length=RSI_SMOOTHING_LENGTH, smoothing_type=RSI_SMOOTHING_TYPE):
    """Adds RSI and zone-related columns based on the provided Pine reference."""
    if df is None or df.empty:
        return df
    ...
    for layer in STOCH_LAYERS:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode='lines',
                name=f"K {label}",
                line=dict(color=layer['k_color'], width=1),
            ),
            row=row_index,
            col=1,
        )
...
def add_macd_panel(fig, df, row_index):
    """Adds traditional MACD panel with histogram, MACD line, signal line, and zero baseline."""
    ...
    histogram_colors = []
    for current, previous in zip(hist, hist_prev):
        previous_val = 0 if pd.isna(previous) else previous

        if pd.isna(current):
            histogram_colors.append('rgba(0,0,0,0)')
        elif current >= 0:
            histogram_colors.append('#FF4D4D' if current >= previous_val else '#F7B6B6')
        else:
            histogram_colors.append('#2F6BFF' if current <= previous_val else '#AFC6FF')
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode='lines',
            name='MACD',
            line=dict(color='#FF3344', width=1),
        ),
        row=row_index,
        col=1,
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode='lines',
            name='Signal',
            line=dict(color='#2F6BFF', width=1),
        ),
        row=row_index,
        col=1,
    )
...
def add_rsi_panel(fig, df, row_index, overbought=RSI_OVERBOUGHT, oversold=RSI_OVERSOLD, midline=RSI_MIDLINE, show_fill=True):
    """Adds RSI panel with overbought/oversold fill zones based on the Pine reference."""
...
def render_chart(df, symbol, display_interval, fetched_interval,
                 show_stochastic=True, show_stoch_fill=True,
                 show_macd=True, show_rsi=True, show_rsi_fill=True):
    """Renders candlestick, volume, and optional indicator panels using Plotly."""
    if df is None or df.empty:
    ...
    if show_stochastic:
        rows += 1
        row_heights = [0.58, 0.16, 0.26] if rows == 3 else row_heights
    ...
    if rows == 2:
        ...
        if show_stochastic and show_macd and not show_rsi:
            row_heights = [0.50, 0.14, 0.20, 0.16]
            volume_row = 2
            stoch_row = 3
            macd_row = 4
        elif show_stochastic and show_rsi and not show_macd:
            row_heights = [0.50, 0.14, 0.20, 0.16]
            volume_row = 2
            stoch_row = 3
            rsi_row = 4
        elif show_macd and show_rsi and not show_stochastic:
            row_heights = [0.50, 0.14, 0.18, 0.18]
            volume_row = 2
            macd_row = 3
            rsi_row = 4
    elif rows == 5:
        row_heights = [0.44, 0.12, 0.17, 0.14, 0.13]
        volume_row = 2
        stoch_row = 3
        macd_row = 4
        rsi_row = 5
    ...
    if show_macd and macd_row is not None:
        add_macd_panel(fig, df, macd_row)

    if show_rsi and rsi_row is not None:
        add_rsi_panel(fig, df, rsi_row, show_fill=show_rsi_fill)
    ...
    # 👉 현재 화면에 표시할 윈도우 범위 (최근 N개 캔들)
    window_size = 150   # 원하는 만큼 조절 (100~300 추천)
...
def main():
    ...
    show_rsi_fill = st.sidebar.checkbox("RSI Fill Zones", value=True, disabled=not show_rsi)
