...
def render_chart(df, symbol, display_interval, fetched_interval, show_stochastic=True, show_stoch_fill=True, show_macd=True):
    """Renders candlestick, volume, and optional indicator panels using Plotly."""
    if df is None or df.empty:
        st.warning("No data to display chart.")
        return

    if show_stochastic and show_macd:
        fig = make_subplots(
            rows=4,
            cols=1,
            shared_xaxes=True,
    ...
    chart_height = 1080 if show_stochastic and show_macd else 900 if (show_stochastic or show_macd) else 700
    fig.update_layout(
    ...
    bottom_row = 4 if show_stochastic and show_macd else 3 if (show_stochastic or show_macd) else 2
