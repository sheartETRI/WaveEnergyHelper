"""Wave Grade Origin UI Playwright."""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wave_grade_origin_ui")
...
def run_screenshots(base_url: str = APP_URL) -> list[str]:
    ...
    with sync_playwright() as p:
        ...
        for symbol, interval in TARGETS:
            ...
            cb = page.locator('[aria-label="Show Grade Origin"]')
            if not cb.is_checked():
                sb.get_by_text("Show Grade Origin", exact=True).click()
            page.wait_for_timeout(2000)
            page.locator('[data-testid="stMainBlockContainer"]').get_by_text(
                "Grade Origin", exact=True,
            ).wait_for(timeout=180_000)
            fpath = os.path.join(OUT_DIR, f"{symbol}_{interval}_grade_origin_on.png")
