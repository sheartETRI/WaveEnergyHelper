"""Wave Grade Early Warning UI Playwright."""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wave_grade_early_warning_ui")
...
def run_screenshots(base_url: str = APP_URL) -> list[str]:
    ...
    with sync_playwright() as p:
        ...
        for symbol, interval in TARGETS:
            ...
            cb = page.locator('[aria-label="Show Grade Early Warning"]')
            if not cb.is_checked():
                sb.get_by_text("Show Grade Early Warning", exact=True).click()
            page.wait_for_timeout(2000)
            page.locator('[data-testid="stMainBlockContainer"]').get_by_text(
                "Grade Early Warning", exact=True,
            ).wait_for(timeout=180_000)
            fpath = os.path.join(OUT_DIR, f"{symbol}_{interval}_early_warning_on.png")
