"""Wave Forward Observation UI Playwright."""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wave_forward_observation_ui")
APP_URL = os.environ.get("WAVE_APP_URL", "http://localhost:8501")
TARGETS = [("BNBUSDT", "4h"), ("ETHUSDT", "1h")]
...
def run_screenshots(base_url: str = APP_URL) -> list[str]:
    ...
    with sync_playwright() as p:
        ...
        for symbol, interval in TARGETS:
            ...
            cb = page.locator('[aria-label="Show Forward Observation"]')
            if not cb.is_checked():
                sb.get_by_text("Show Forward Observation", exact=True).click()
            page.wait_for_timeout(2000)
            page.locator('[data-testid="stMainBlockContainer"]').get_by_text(
                "Forward Observation", exact=True,
            ).wait_for(timeout=180_000)
            fpath = os.path.join(OUT_DIR, f"{symbol}_{interval}_forward_obs_on.png")
