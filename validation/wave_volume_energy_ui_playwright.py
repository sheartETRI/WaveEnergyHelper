"""Wave Volume Energy UI Playwright."""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wave_volume_energy_ui")
...
def run_screenshots(base_url: str = APP_URL) -> list[str]:
    ...
    with sync_playwright() as p:
        ...
        for symbol, interval in TARGETS:
            ...
            cb = page.locator('[aria-label="Show Volume Energy"]')
            if not cb.is_checked():
                sb.get_by_text("Show Volume Energy", exact=True).click()
            page.wait_for_timeout(2000)
            page.locator('[data-testid="stMainBlockContainer"]').get_by_text(
                "Volume Energy", exact=True,
            ).wait_for(timeout=180_000)
            fpath = os.path.join(OUT_DIR, f"{symbol}_{interval}_volume_energy_on.png")
