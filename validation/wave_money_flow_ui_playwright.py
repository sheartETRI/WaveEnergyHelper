...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wave_money_flow_ui")
...
def run_screenshots(base_url: str = APP_URL) -> list[str]:
    ...
    with sync_playwright() as p:
        ...
        for symbol, interval in TARGETS:
            ...
            cb = page.locator('[aria-label="Show Money Flow"]')
            if not cb.is_checked():
                sb.get_by_text("Show Money Flow", exact=True).click()
            page.wait_for_timeout(2000)
            page.locator('[data-testid="stMainBlockContainer"]').get_by_text(
                "Money Flow", exact=True,
            ).wait_for(timeout=180_000)
            fpath = os.path.join(OUT_DIR, f"{symbol}_{interval}_money_flow_on.png")
