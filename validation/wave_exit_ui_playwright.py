...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wave_exit_ui")
    ...
    with sync_playwright() as p:
        ...
        for symbol, interval in TARGETS:
            ...
            for label, val in [("Symbol", symbol), ("Timeframe", interval)]:
                ...
                combo = page.locator(f'[aria-label*="{label}"]').first
            ...
            cb = page.locator('[aria-label="Show Wave Exit"]')

            if not cb.is_checked():

                sidebar.get_by_text("Show Wave Exit", exact=True).click()

            page.wait_for_timeout(2000)

            page.locator('[data-testid="stMainBlockContainer"]').get_by_text(

                "Wave Exit", exact=True,

            ).wait_for(timeout=180_000)

            fpath = os.path.join(OUT_DIR, f"{symbol}_{interval}_wave_exit_on.png")
