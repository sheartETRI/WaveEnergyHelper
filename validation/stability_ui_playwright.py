"""Stability Verdict UI Playwright 스크린샷.

실행 (Streamlit 별도 기동):
  streamlit run main.py --server.port 8501
  python validation/stability_ui_playwright.py
"""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "stability_ui")
...
def _set_stability_checkbox(page, enabled: bool) -> None:
    sidebar = page.locator('[data-testid="stSidebar"]')
    sidebar.get_by_text("Show Stability Verdict", exact=True).click()
    cb = page.locator('[aria-label="Show Stability Verdict"]')
...
def run_screenshots(base_url: str = APP_URL) -> list[str]:
    ...
    with sync_playwright() as p:
        ...
        for symbol, interval in TARGETS:
            ...
            for flag, suffix in ((False, "off"), (True, "on")):
                t0 = time.perf_counter()
                _set_stability_checkbox(page, flag)
                if flag:
                    page.locator('[data-testid="stMainBlockContainer"]').get_by_text(
                        "Stability Verdict", exact=True,
                    ).wait_for(timeout=120_000)
                    page.wait_for_timeout(2000)
                elapsed = time.perf_counter() - t0
                fname = f"{symbol}_{interval}_stability_{suffix}.png"
