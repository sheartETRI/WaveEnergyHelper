"""Wave Confirmation UI Playwright 스크린샷.
...
실행 (Streamlit 별도 기동):
  ...
  python validation/wave_confirmation_ui_playwright.py
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wave_confirmation_ui")
...
def _set_confirmation(page, enabled: bool) -> None:

    sidebar = page.locator('[data-testid="stSidebar"]')

    sidebar.get_by_text("Show Wave Confirmation", exact=True).click()

    cb = page.locator('[aria-label="Show Wave Confirmation"]')
...
def run_screenshots(base_url: str = APP_URL) -> list[str]:
    ...
    with sync_playwright() as p:
        ...
        for symbol, interval in TARGETS:
            ...
            fname = f"{symbol}_{interval}_wave_confirmation_on.png"
