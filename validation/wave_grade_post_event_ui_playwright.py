"""Wave Grade Post-Event UI Playwright."""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wave_grade_post_event_ui")
...
def run_screenshots(base_url: str = APP_URL) -> list[str]:
    ...
    with sync_playwright() as p:
        ...
        for symbol, interval in TARGETS:
            ...
            cb = page.locator('[aria-label="Show Grade Post Event"]')
            if not cb.is_checked():
                sb.get_by_text("Show Grade Post Event", exact=True).click()
            page.wait_for_timeout(2000)
            page.locator('[data-testid="stMainBlockContainer"]').get_by_text(
                "Grade Post Event", exact=True,
            ).wait_for(timeout=180_000)
            fpath = os.path.join(OUT_DIR, f"{symbol}_{interval}_grade_post_event_on.png")
