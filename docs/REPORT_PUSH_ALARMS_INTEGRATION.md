...
## 9. 추가: 매일 09:00 KST 요약 알림 (2026-09-23, 커밋 1개)

- 시각: `daily_summary_due(hist, now)` — 이력 `daily.last_date`(UTC 날짜 키) ≠ 오늘(UTC) 이면 due. 00:00 UTC(09:00 KST) 경계로 날이 바뀌므로
  그날 첫 순회(09:01)에 1회, 같은 날 재순회는 없음, 서버가 늦게 켜져도 그날 첫 순회에 1회. 발송 성공 때만 기록(실패·토큰 없음 → 다음 순회 재시도).
  이력 형식에 `daily` 필드 추가(`notify/history.py` — signal-alarm 전용 확장, main 사본과 다름).
- 내용: 첫 줄 = `headline()` — ① 지난 24h 60MA 상방·하방 전환(TF 당 최신 1건, 긴 TF부터, 종류·시각·다이버전스 병기) → ② 대기 중 후보(상승 쪽
  `track_candidates` 최근 120봉의 '대기 중' 행, ★ 우선·경과 짧은 순, 긴 TF부터) → ③ "특이 사항 없음". 상세 4줄: 지난 24h 실제 발송 건수(이력
  delivered=True, 종류별) · 대기 중 목록 · 60MA 방향(MA60 마지막 vs 직전, ↑/↓/→) · 스캐너 상태(실패 셀 수, 마지막 순회 KST, OK 셀 수).
  제목 `[WEH] 일일 요약 09-23 (미검증)`. 평가어("중요/주목/유망") 부재를 테스트가 단언. 대기 중 후보는 후보 알림 종류(쌍바닥)와 같은 상승 쪽만 센다.
- 순회 안 위치: 알림 발송·ledger 뒤(오늘 발송분까지 센다) → 이력 저장 전. 발송 조건·검출 무접촉(순회가 만든 이벤트·추적 표·MA60 컬럼만 읽음).
- dry-run 샘플(이 PC, 2026-09-23 02:14 KST, 빈 이력):
  ```
  [WEH] 일일 요약 09-23 (미검증)
  볼 TF: 2h — 후보 대기 중 (경과 17/20)
  지난 24h 발송: 후보 0 · 상방 0 · 하방 0
  대기 중: 2h(17/20)
  60MA: 1h↑ 2h↑ 4h↑ 6h↑ 12h↑ 1d↑ 2d↑
  스캐너 정상 · 마지막 순회 02:14 · 7셀 OK
  ```
- 테스트 `tests/test_push_daily_summary.py` 7건: 00:00 UTC 경계·당일 1회·늦은 기동·중복 방지, daily 필드 왕복, 첫 줄 우선순위(24h 경계 포함)·
  다중 TF 정렬·시각 라벨, 본문 5줄 형식·실패 셀 표기·금지 어휘, 셀 스냅샷 = 추적 표, run 통합(dry-run 샘플·1회 발송·실패 재시도·토큰 없음),
  '확정 시 이미 상방' 후보 제외(추적 표 상태 '해당 없음 (이미 상방)' → 대기 집계·첫 줄 ②에서 빠지고 "대기 중: 없음 (이미 상방 1건 별도)" 로 건수만,
  첫 줄 "특이 사항 없음" — 908b71c, 제외는 표시 모듈 `MT.waiting_rows`·`MT.summarize` 한 곳).
  기존 run 테스트는 그날 요약을 이미 보낸 상태로 심어 알림 건수만 본다.
- **apolo 적용: `git pull` 후 `sudo systemctl restart weh-push` 필요**(스크립트·notify/history 변경).
