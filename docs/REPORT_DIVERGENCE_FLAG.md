...
| B | `display/divergence_flag.py` 신설 + 60MA 상방 전환 추적 표 '다이버전스' 열 + 코호트 집계 줄 + 테스트 | signal-alarm | `7f10312` |
...
| C | 후보 알림 `stoch_db` + 60MA 상방 전환 알림 다이버전스 줄 + 전방 ledger(1h·4h·6h·1d) + 테스트 35건 | main | `a95b99c` (푸시 완료, 추적 클론 동기화 완료) |
...
- 60MA 상방 전환 추적 표에 **'다이버전스' 열**(확정 시 60MA 뒤, 있음/없음). 하방 추적 표는 변경 없음.
...
### 후속: "재시작했는데 열이 안 보인다" — 원인과 재발 방지 (2026-09-22 05:50)

- **원인은 코드 경로가 아니라 옛 프로세스**였다. 8501 에는 03:10 에 뜬 인스턴스(pid 17332, 기동 HEAD `bc4f001`)가 살아 있었고, 05:29 의
  "재시작"(pid 2784, HEAD `f332a0c`)은 같은 8501 에 **중복 바인드**됐다(Windows 에서 SO_REUSEADDR 로 두 프로세스가 같은 포트를 잡을 수 있고,
  연결은 그중 하나 — 여기서는 옛 프로세스 — 로 간다). 8501 을 열어 사이드바 코드 버전 위젯을 읽으니 `기동 HEAD bc4f001 · 03:10:44` 였다.
  옛 프로세스(17332·래퍼 10064)를 종료하자 8501 이 새 프로세스로 넘어갔고(위젯 `f332a0c`), 표에 '다이버전스' 열(그리드 12열)과 코호트
  캡션이 렌더됐다. 재시작 시 **이전 프로세스가 죽었는지(포트 소유 pid)와 사이드바의 기동 HEAD** 를 확인해야 한다.
- 재발 방지 테스트 `tests/test_page_render_smoke.py`(2건): `streamlit.testing.v1.AppTest` 로 main.py 를 실제 실행(네트워크는 `data.binance.requests.get`
  을 합성 kline 으로 대체 — 렌더 경로의 유일한 네트워크 지점)해, 렌더 트리의 `st.dataframe` 중 60MA 상방 전환 추적 표의 열 목록이 `COLUMNS` 와 같고
  '다이버전스' 열에 있음·없음이 모두 있으며, "다이버전스 있음: 전환 … (미검증, 표본 적음)" 캡션이 '최근 120봉' 집계 바로 다음에 있음을 단언.
  렌더된 값이 같은 프레임의 `track_candidates` 직접 계산과 일치함도 확인. 하방 표에는 열이 없음. 시드 2(최근 120봉에 상승 후보 5건이 들어오는
  합성 시계열 — 표가 비면 그려지지 않으므로). 실행 ≈17초.
- 부수 관찰: AppTest 실행 시 Streamlit 1.58 이 `st.components.v1.html` → `st.iframe` 교체 경고를 낸다(2026-06-01 이후 제거 예고, 아직 동작). LW 차트 임베드가 대상 — 별도 결정.
...
- 메시지(테스트 고정, 위임 예시 그대로):
  ...
  ```
  3행은 상태에 따라: 확정 시 이미 상방 → "60MA 이미 상방"(발송함) · 스캔 시점에 이미 전환 → "60MA 상방 전환 발생 mm-dd hh:mm (소요 N봉)" · 창 종료 → "소멸 (창 20봉 안 60MA 전환 없음)". (Actions 가 하루 3회 정도만 도는 현 상황에서는 확정 뒤 몇 봉 지나 스캔되는 일이 흔해 상태 표기가 필요했다.)
...
- 기존 60MA 상방 전환 알림은 3줄 유지 + 끝에 "다이버전스 있음/없음" 1줄.
...
기존 알림(60MA 상방 전환 0.72/일 + 하방 전환 0.65/일 + 구조 LL)까지 합치면 텔레그램 하루 3건 이상 — 1h 후보 알림을 뺄지는 김박사 결정(뺀다면 `TFS` 가 아니라 `stoch_db` 종류의 TF 필터 1줄).
