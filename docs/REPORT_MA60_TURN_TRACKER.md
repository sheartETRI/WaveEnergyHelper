# signal-alarm 브랜치 — 60MA 상방 전환 추적 표시 (미검증 · 관측 전용) 보고
...
## 3. 표시 내용 (알람 탭 별도 섹션 "60MA 상방 전환 추적 (미검증)" — 2026-09-23 하방 섹션과 대칭 표기로 변경, 내부 식별자·이력 키 불변)
...
- 표(최근 120봉 안에 가용된 후보, 최신순): 상태 · 확정 시각 · 경과(N/20) · 60MA 현재 · 확정 시 60MA(**'이미 상방'** 별도 표기)
  ...
  - '이미 상방'(확정 시 60MA 가 이미 상방) 후보는 상태를 **'해당 없음 (이미 상방)'** 으로 두고(창 진행 중이든 끝났든 같음) 대기·소멸·전환율에서
    ...
    60MA 상방 전환 알림은 종전과 같다(`already_up` 필드로 구분). 차트 기준선도 '대기 중' 행만 긋는다.
...
방식(가격선 + 축 뱃지, 차트 안 라벨 없음)으로 그린다. 캡션 조각 " · 상방 추적 ─ 저점 76,123 · ┄ 기준선 75,743 (미검증)"
